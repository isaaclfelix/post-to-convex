...
**PHP** follows [WordPress Coding Standards](https://github.com/WordPress-Coding-Standards/WordPress-Coding-Standards) plus repo-wide **type-safety** rules (`TypeSafety` ruleset: Slevomat strict types and native hints, Squiz function PHPDoc) via PHPCS/PHPCBF — see [README.md](./README.md#php-coding-standards). Post-to-convex–specific rules (prefixes, i18n, PSR-4 class sniff) apply only under `wp-content/plugins/post-to-convex/`. After adding a PHPCS dependency, append its vendor path to `installed_paths` in `.phpcs.xml.dist`. PHP is not run in the pre-commit hook; run `./bin/php-lint.sh` (or `composer run lint:php` in Docker) before pushing PHP changes. Make sure the PHP coding standards are configured at the IDE level (see README.md).

On Windows, run PHP lint scripts from **Ubuntu WSL**, not PowerShell. If a `bin/*.sh` script fails with `$'\r': command not found`, normalize line endings first: `sed -i 's/\r$//' bin/*.sh` (see [Shell scripts (`bin/*.sh`)](./README.md#shell-scripts-binsh) in the README).
